...
function getSelectedIndexesFromURL() {
  const urlParams = new URLSearchParams(window.location.search);
...
function updateSelectedInURL(seed) {
  const selected = [];
...
function createBingo(seed, selectedIndexes = []) {
...
(_a = document.getElementById("generate")) === null || _a === void 0
  ? void 0
...
const seed =
  (_b = getSeedFromURL()) !== null && _b !== void 0 ? _b : generateNewSeed();
const selected = getSelectedIndexesFromURL();
createBingo(seed, selected);
(_c = document.getElementById("copy-button")) === null || _c === void 0
  ? void 0
  ...
  targetPoints.forEach((target, b) => {
    setTimeout(() => {
      ...
      for (let i = 0; i < focusedSplatsPerBurst; i++) {
        const originX =
          center.x + (Math.random() * originWobble - originWobble / 2);
        const originY =
          center.y + (Math.random() * originWobble - originWobble / 2);
